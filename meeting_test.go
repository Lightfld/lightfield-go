...
func TestMeetingDeleteWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Meeting.Delete(
		...
		githubcomlightfldlightfieldgo.MeetingDeleteParams{
			Body: githubcomlightfldlightfieldgo.MeetingDeleteParamsBody{},
		},
	)
	if err != nil {
		var apierr *githubcomlightfldlightfieldgo.Error
...
func TestMeetingDefinitions(t *testing.T) {
	...
	_, err := client.Meeting.Definitions(context.TODO())
	if err != nil {
		var apierr *githubcomlightfldlightfieldgo.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
