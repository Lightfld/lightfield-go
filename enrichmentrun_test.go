...
func TestEnrichmentRunEnrichWithOptionalParams(t *testing.T) {
	...
	_, err := client.EnrichmentRun.Enrich(
		context.TODO(),
		"entityId",
...
func TestEnrichmentRunGetEnrichmentRun(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.EnrichmentRun.GetEnrichmentRun(context.TODO(), "runId")
	if err != nil {
		var apierr *githubcomlightfldlightfieldgo.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
